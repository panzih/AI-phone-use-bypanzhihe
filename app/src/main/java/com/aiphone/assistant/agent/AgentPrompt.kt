...
**优先用 index 指定要操作的元素。**
只有当目标在列表里找不到（比如列表没截全、或者是个纯图标的自绘控件）时，才退回用 x / y 直接给坐标。
...

{
  ...
  "index": 3,
  "x": 0, "y": 0, "x2": 0, "y2": 0,
  "duration_ms": 0, "direction": "", "text": "", "package": "",
  "next_hint": "做完这一步之后，接下来要做什么（一句话，给用户看的）",
  "finished": false, "summary": ""
}

任务完成时，把 finished 设为 true，并在 summary 里写清楚结果，action 留空。
...
2. 动作执行完会有反馈。如果画面没变化，说明这个动作没生效，**换一种方式**，不要重复同一个动作。
3. 完成用户的任务就立刻停下来并设 finished=true，不要顺手多做别的事。
4. 如果连续几步都推不动，直接在 summary 里说明卡在哪里、需要用户做什么，然后设 finished=true。
5. 不要操作需要付款、转账、删除数据的按钮。
""".trimIndent()

    fun stepMessage(step: Int, maxSteps: Int, task: String, uiTree: String?,
                    interruption: String? = null, lastResult: String? = null): String = buildString {
        appendLine("当前是第 $step / $maxSteps 步。")
        appendLine("用户的任务：$task")
        if (!lastResult.isNullOrBlank()) appendLine("上一步的执行结果：$lastResult")
        if (!interruption.isNullOrBlank()) { appendLine(); appendLine("⚠️ $interruption") }
        appendLine()
        if (uiTree.isNullOrBlank()) { appendLine("# 界面元素"); appendLine("（这次没能读到控件树，请直接用截图判断，并用 x/y 给坐标）") }
        else { appendLine("# 界面元素"); appendLine(uiTree) }
        appendLine()
        append("请输出下一个动作的 JSON。")
    }

    fun describeAction(kind: TouchKind, index: Int, x: Int, y: Int, x2: Int, y2: Int,
                       direction: ScrollDirection?, text: String, pkg: String, durationMs: Int): String = when (kind) {
        ...
        TouchKind.SWIPE, TouchKind.FLICK, TouchKind.DRAG -> "${kind.label} ($x,$y)→($x2,$y2) ${durationMs}ms"
