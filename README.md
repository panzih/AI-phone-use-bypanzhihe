...
| 模型 | `deepseek-flash` |
| 思考模式 | 一个总开关：**关（默认）**时使用 HIGH 档（充分推理）；打开后露出 OFF / LOW / HIGH / MAX 滑块自选。见下面「思考模式」一节 |
...
普通情况下 AI 在主屏用无障碍直接操作，会"占着"屏幕。副屏功能用 Shizuku
借到 shell 身份，另开一块**虚拟屏幕**，把目标 app 整个搬过去、AI 在那块屏上
继续干活；主屏回到桌面，留给你自己用。

**需要先装 [Shizuku](https://shizuku.rikka.app/) 并启动它**（无 root 设备用
「无线调试」启动，重启后要重新启动一次）。Shizuku 就绪后，任务运行时悬浮窗上
会出现「切到副屏」按钮。

### 手动切过去 / 切回来

任务跑起来后，点悬浮窗上的「切到副屏」：立刻建一块虚拟屏、把当前 app 整栈
迁过去，主屏回桌面。想拉回来时，点悬浮钮——它此时变绿、写着「切回主屏」，
app 整栈回迁、副屏销毁，任务编号不变。

### 回桌面自动迁出

任务运行中你直接按 HOME 回桌面，纸盒会检测到，自动建一块副屏、把刚才那个
app 移出去接着跑，不用手动点。检测带约 1 秒防抖，并有多重护栏：只有在任务
真正运行、Shizuku 就绪、且刚才确实在某个 app 里时才会触发；自己在桌面点图标
打开 app、或在纸盒界面按 HOME 都不会误迁。

反过来也成立：**跑在副屏上时，你打开纸盒，它会自动把任务搬回主屏并把目标
app 弹到前台**，你不用去点悬浮窗上那个绿按钮。搬完有 30 秒静默期，避免
"按 HOME 又切回去"打成乒乓球 —— 这段时间里手动按悬浮钮仍然有效。

（这条只在「你确实从别处回到纸盒」时才触发；任务刚启动时那次界面刷新不算。
另外从镜像页按返回键回到纸盒主界面也算"回来了"。）

### 有一件事必须说清楚

**副屏上的点击做不到用无障碍实现。** 安卓的手势注入接口（`dispatchGesture`）
没有"指定哪块屏幕"的参数，只能作用于主屏。所以副屏的触控只能走 shell 的
`input -d <屏幕id>` —— **副屏功能必须依赖 Shizuku**，无障碍在这一块帮不上忙。

而且**读也读不到**（2026-09-26 实测确认，不只是推测）：

无障碍服务能**看到**副屏存在（`getWindowsOnAllDisplays()` 返回的 map 里有它），
但**拿不到上面任何窗口**。同一个时刻，系统的 WindowManager 说副屏上有 2 个窗口，
无障碍报的是 0 个 —— 所以也就拿不到控件树、读不到元素编号。

结论：副屏**只能靠「截图 + 坐标」**，这不是没做，是安卓不给。
（测法：在副屏上放一个真实窗口，然后同时看 `dumpsys window windows` 和无障碍探针
的输出做对比 —— 只看探针会误判，因为默认它只列出"有窗口的屏"。）

### 副屏模式比主屏弱在哪

| | 主屏（无障碍） | 副屏（Shizuku） |
|---|---|---|
| 控件树 | **有**，模型选编号定位 | **没有**（安卓不给第三方读副屏的无障碍树，已实测确认） |
| 定位方式 | 编号优先，坐标兜底 | 只能看截图给坐标 |
| 中文输入 | `ACTION_SET_TEXT`，随便输 | `input text` 只支持 ASCII，**中文输不了** |
| 重启后 | 自动恢复 | 要重新启动 Shizuku |

所以：**需要精确操作的复杂界面留在主屏；那种"跑起来别打扰我"的流程适合丢去
副屏。** 副屏读不到控件树时，每一步会自动带上截图（没有别的信息来源），这是
它必然的成本。
...
**思考模式**单独说一下，因为它直接决定速度和花费。它是一个总开关：

- **关（出厂默认）**：显式使用 **HIGH** 档（`thinking:enabled` + `reasoning_effort:high`），
  充分推理，复杂任务更稳，但不是最省的。
- **打开**：下面露出一个四档滑块，自己定强度：

| 档位 | 行为 |
|---|---|
| OFF | 不推理，直接给动作。最快最省，适合「点哪个按钮」这类简单判断 |
| LOW | 少量推理，速度接近 OFF，复杂界面比 OFF 稳 |
| HIGH | 充分推理，慢一些、贵一些 |
| MAX | 推理拉满，最慢最贵 |

自动化操作大多是「看一眼，决定点哪」。**想又快又省，就打开开关选 OFF**；
界面复杂、要绕圈子时再选 HIGH / MAX。平时图省事可以让开关关着、用默认 HIGH。

> 界面上显示什么，实际发出去的就是什么：开关关着时发 HIGH 档参数，
> 不会出现"以为没开思考、其实在最贵档位"的情况。
>
> 这套参数（`thinking` / `reasoning_effort`）是 DeepSeek 的写法。换成别的
> 服务商如果报错，打开开关后选 OFF —— 只有 OFF 不发思考参数，普通模型都认。
...
1. **思考模式**（设置 → 模型）。**默认是 HIGH**（充分推理），想又快又省就
   打开开关选 OFF。往上调（LOW / HIGH / MAX）会让每一步都多推理一遍，
   速度变慢、费用上升 —— 界面复杂、AI 老走错路的时候再试 HIGH / MAX。
2. **最大步数**就是最坏情况下的花费上限。

**默认不发送图片**也是省钱的关键 —— 一张 1080×2400 的手机截图，压成全分辨率
JPEG（q82）之后也有几百 KB，base64 编码还要再胖三分之一；如果每一步都发，
token 会翻好几倍。所以只有模型确实看不清界面时才会要一张 ——
而且**它会自己判断**：看不懂这一屏时才要，或者顺手在动作里插一步截图。

心疼钱就把「最大步数」调小；「上下文缓存命中率」可以在导出的日志里看到。

**怎么读那个命中率**：任务结束时那行「上下文缓存：命中 X / 未命中 Y」是**整个任务
累计**的，所以**越短的任务数字越低**，这正常 —— 每步都会新增两条消息（模型上一步
的回复 + 这一步的新界面），这两条按定义必然未命中。

真正该看的是：**每一步的「缓存命中」是不是正好等于上一步的「输入总量」**。
是 → 每一轮都完整复用了上一轮，已经是最优；不是 → 前缀被打断了（这时候
数字会突然掉下去，但不会报错，只会悄悄多花钱）。

5 步的小任务整体六十几、十几步的能到九十几，都可能是完全正常的。

### 一分钱都不想花：桥接模式（模型换成"你自己"）

「接口地址」填的是任意 **OpenAI 兼容端点**，所以模型这一环是可以整个拿掉的。

仓库里的 `tools/bridge_llm.py` 就是一个"假模型服务"：它不产生任何回复，而是把每次
请求**落成文件**，然后等你（或另一个 AI）在另一个文件里写回复。

```bash
python3 tools/bridge_llm.py           # 起服务（默认 8766）
python3 tools/bridge_ctl.py status    # 收到几条、回了几条
python3 tools/bridge_ctl.py next      # 打印最早一条还没回的请求（可读摘要）
python3 tools/bridge_ctl.py reply 3 '{"actions":[{"action":"open_app","package":"com.android.settings"}]}'
```

纸盒里「设置 → 模型 → 接口地址」填你电脑的地址（模拟器是 `http://10.0.2.2:8766`，
...
> 人读不动。`req-NNNN.md` 只留"这一步要判断的东西"（步号、任务、上一批动作的
> 结果、界面元素编号列表），附带的截图单独存成 `req-NNNN.jpg` 可以直接看。
...
| 三指以上的自定义手势 | 暂未支持 |
| 副屏只能截图 + 坐标点击 | 系统不把虚拟屏的窗口交给无障碍，读不到控件、需 Shizuku；详见「副屏」一节 |
