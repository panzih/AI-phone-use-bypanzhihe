...
| 模型 | `deepseek-flash` |
| 图片精度 | `original`（**别选 low**，界面上的字会糊到看不清） |
...
| 项 | 默认 | 说明 |
|---|---|---|
| 最大步数 | 30 | 一次任务最多走多少步。每步都要调一次模型，**这个值就是你的花费上限** |
| 自动清空上下文 | 不清空 | 多久没发消息就把上下文清掉 |
| 开启记忆 | 关 | 让 AI 把对话沉淀成「用户洞察」存下来 |
| 保存记忆 | 开 | 清空上下文前先存成文件，而不是直接丢掉 |
| 保存日志 | 开 | 每次任务写一份完整日志 |
...
**默认不发送图片**是省钱的关键 —— 一张 1080×2400 的手机截图编码后有一两兆，
