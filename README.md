...
代码已经接好（建屏 / 按屏截图 / 按屏注入 / 镜像窗口），但这套命令在
不同 ROM 上的表现不一样，**我这边没有真机可验证**。所以副屏页面被做成
一个"按顺序点一遍"的自检流程：
...
每一步都会把**命令的原始输出**摊在下面。哪一步不成立，把那段输出发出来
就能定位。三步都成立之后，下一步才是"让 AI 直接在副屏上跑任务"。
...
| 档位 | 行为 | 什么时候用 |
|---|---|---|
| 每次重置 | 每发一条新任务就开全新上下文 | 任务之间互不相关，想让每次判断都干净 |
| 超过 24 小时 | 24 小时没动静才清 | 日常用这档：连着做几件事能接上，隔天又从头开始 |
| 不限 | 永远不清 | 一段长流程要连着做很多步 |
| 保存日志 | 开 | 每次任务写一份完整日志 |
...
**记忆是怎么工作的**（想用的话在「开发者设置」里打开「开启记忆」）：

- **只增不减**：全部记忆在一个文件（应用私有目录的 `纸盒/memory.md`）里，
  每次任务结束由 AI 归纳一条，**追加到末尾**。不删不改 —— 这样 AI 不会
  把用户纠正过的错误认知又改回去，你也能随时打开文件看完整的来龙去脉
- **只在「新开上下文」时注入**：模型一开始是看不到你的记忆的。
  只有上下文重开的那一刻，记忆才会被放进系统提示词。原因是服务端的
  上下文缓存按前缀匹配，往一段正在进行的对话中间插记忆会把前缀整段作废，
  下一次请求全部按未命中计价 —— 那比省下的那点 token 贵得多
- **上下文没重开时**：模型需要就自己调 `recall_memory` 技能拿完整版
